...
import top.omooo.booster.kotlinx.file
import top.omooo.booster.kotlinx.int
import top.omooo.booster.kotlinx.separatorsToSystem
import top.omooo.booster.kotlinx.touch
import java.io.File
...
class ResolvedArtifactResults(private val variant: BaseVariant) :
    ...
    init {
        results = listOf(AndroidArtifacts.ArtifactType.AAR, AndroidArtifacts.ArtifactType.JAR)
            .asSequence()
            .map {
                variant.variantData.scope.getArtifactCollection(
    ...
    override fun containsAll(elements: Collection<ResolvedArtifactResult>) =
        results.intersect(elements).size == elements.size
     ...
     */
    private val output = variant.variantData.scope.globalScope.intermediatesDir.file("dependencies")
        .file(variant.dirName.separatorsToSystem()).file("dependencies.txt")

    /**
     * Default dependency stringify
     */
    private val stringify: (ResolvedArtifactResult) -> String = { result ->
        result.id.componentIdentifier.displayName + " ".repeat(maxNameWidth + 1 - result.id.componentIdentifier.displayName.length) + result.file + " ".repeat(
            maxFileWidth + 1 - result.file.path.length
        ) + result.file.length()
    }
    ...
    fun dump(
        file: File = output,
        stringifier: (ResolvedArtifactResult) -> String = this.stringify
    ) {
     ...
     */
    fun print(
        printer: PrintWriter = PrintWriter(System.out, true),
        stringifier: (ResolvedArtifactResult) -> String = this.stringify
    ) {
        forEach { result ->
            printer.apply {
